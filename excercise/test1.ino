...
#define MOTOR_BACKWARD_PIN 13  // Motor direction pin for backward
#define MOTOR_SPEED_PIN 15     // PWM speed control pin
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  if (strstr(variable, "setSpeed_forward") != NULL) {
    ...
    // if (!strcmp(variable, "moveCamera_up")) {
    //   if (tiltPos <= 170) {
    //     tiltPos += 10;
    //     tiltServo.write(tiltPos);
    //   }
    ...
    if (!strcmp(variable, "moveCar_left")) {
      if (steeringPos <= 170) {
        steeringPos += 10;
        steeringServo.write(steeringPos);
      }
...
void moveCarForwardWithSpeed(int speed) {
  ...
  // For example, set PWM or motor driver speed
